...
sed -i '/include \/etc\/nginx\/conf\.d\/\*\.conf;/a \}#tmp_id_5' /etc/nginx/nginx.conf

sed -i '/include \/etc\/nginx\/conf\.d\/\*\.conf;/a US yes;' /etc/nginx/nginx.conf

sed -i '/include \/etc\/nginx\/conf\.d\/\*\.conf;/a default no;' /etc/nginx/nginx.conf

sed -i '/include \/etc\/nginx\/conf\.d\/\*\.conf;/a map \$geoip_country_code \$allowed_country \{' /etc/nginx/nginx.conf

sed -i '/include \/etc\/nginx\/conf\.d\/\*\.conf;/a \

' /etc/nginx/nginx.conf

sed -i 's/\}#tmp_id_5/    \}#tmp_id_5/' /etc/nginx/nginx.conf

sed -i 's/US yes;/        US yes;/' /etc/nginx/nginx.conf

sed -i 's/default no;/        default no;/' /etc/nginx/nginx.conf

sed -i 's/map \$geoip_country_code \$allowed_country/    map \$geoip_country_code \$allowed_country/' /etc/nginx/nginx.conf

sed -i '/^        include \/etc\/nginx\/default\.d\/\*\.conf;/a \}#tmp_id_1' /etc/nginx/nginx.conf

sed -i '/^        include \/etc\/nginx\/default\.d\/\*\.conf;/a return 444;' /etc/nginx/nginx.conf

sed -i '/^        include \/etc\/nginx\/default\.d\/\*\.conf;/a if (\$allowed_country = no) \{' /etc/nginx/nginx.conf

sed -i '/^        include \/etc\/nginx\/default\.d\/\*\.conf;/a \

' /etc/nginx/nginx.conf

sed -i 's/\}#tmp_id_1/        \}#tmp_id_1/' /etc/nginx/nginx.conf

sed -i 's/return 444;/            return 444;/' /etc/nginx/nginx.conf

sed -i 's/if (\$allowed_country = no)/        if (\$allowed_country = no)/' /etc/nginx/nginx.conf

sed -i '/^        include \/etc\/nginx\/default\.d\/\*\.conf;/a access_log \/var\/log\/nginx\/server-block-1-access\.log  timekeeper;' /etc/nginx/nginx.conf

sed -i 's/access_log \/var\/log\/nginx\/server-block-1-access\.log  timekeeper;/        access_log \/var\/log\/nginx\/server-block-1-access\.log  timekeeper;/' /etc/nginx/nginx.conf

sed -i '/access_log \/var\/log\/nginx\/server-block-1-access\.log  timekeeper;/a error_log \/var\/log\/nginx\/server-block-1-error\.log;' /etc/nginx/nginx.conf

sed -i 's/error_log \/var\/log\/nginx\/server-block-1-error\.log;/        error_log \/var\/log\/nginx\/server-block-1-error\.log;/' /etc/nginx/nginx.conf
...
sh -c "echo 'gzip on;' >> /etc/nginx/conf.d/gzip.conf"

sh -c "echo 'gzip_disable \"msie6\";' >> /etc/nginx/conf.d/gzip.conf"
